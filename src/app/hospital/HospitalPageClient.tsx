...
import type { HospitalMaster } from '@/lib/types';
import { HospitalSapLevel, HospitalSizeLevel } from '@/lib/types';
...
const HospitalPageClient: React.FC<HospitalPageProps> = ({ data }) => {
  const [activeTab, setActiveTab] = useState<TabType>('SAP');
  const mapContainerRef = useRef<HTMLDivElement>(null);
  const mapInstanceRef = useRef<Leaflet.Map | null>(null);
  const leafletModuleRef = useRef<typeof Leaflet | null>(null);
  ...
  useEffect(() => {
    ...
    const initMap = async () => {
      ...
      if (!mapContainerRef.current || mapInstanceRef.current || cancelled) return;
      const map = leaflet.map(mapContainerRef.current).setView([16.95, 100.5], 9);

      const streetMap = leaflet.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
      ...
      data.forEach((h) => {
        if (h.lat && h.lng) {
          leaflet
            .marker([h.lat, h.lng], { icon: hospitalIcon })
            .addTo(map)
            .bindPopup(`
                    <div class="font-sans">
  ...
  const handleFocusHospital = (lat?: number | null, lng?: number | null) => {
